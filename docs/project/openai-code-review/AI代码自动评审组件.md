...
title: AI代码自动评审组件
date: 2026-08-06
description: 一个以 独立可运行 Fat Jar 形式交付的 AI 代码评审组件：在 GitHub Actions 中被触发后，自动提取最新一次提交的变更（diff），调用阿里云百炼大模型进行评审，并把评审意见以 Markdown 日志的形式提交到专用的日志仓库，形成可追溯的评审记录。
tags: [SDK, 大模型]
---
# AI代码自动评审 (Base on OpenAI)

项目地址：https://github.com/ObjectGeeker/openai-code-review
